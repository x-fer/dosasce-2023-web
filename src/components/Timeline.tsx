  ...
  return (
    ...
    >
      ...
      <div
        ...
        // onClick={isLive ? () => navigate(`/problem/${1}`) : undefined}
      >
        ...
        {isLive ? (
          isFinished ? (
            ...
            <p className="paragraph flex items-center text-2xl text-red underline">
              Otvara se {props.startDate?.toLocaleDateString('hr', { day: 'numeric', month: 'long' })}!
            </p>
          )
        ) : (
          <>
...
const PrimjerCard = (props: TimelineCardProps) => {
  // const isLive = props.startDate && props.endDate && new Date() >= props.startDate && new Date() <= props.endDate
  const isLive = useZadActive(props.number ?? 0)
  const isFinished = useZadFinished()
  const navigate = useNavigate()
        ...
        <div className="w-full bg-snow-pattern">
          <div className="relative flex flex-col items-center justify-center px-4">
            ...
            <TimelineCard
              ...
              image={Snowman}
            />
            <TimelineCard name="Dodjela" startDate={dodjela.startDate} image={Presents} left/>
          </div>
        </div>
      </div>
    </>
