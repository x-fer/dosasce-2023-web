...
const SolutionBox2 = ({ number }: { number: number }) => {
  ...
  const provjeriRjesenje = async () => {
    ...
    const problemID = getProblemID(category as string, number)
        ...
        <div className="flex flex-row gap-2 text-start md:flex-col md:gap-3">
          <button
            ...
            className={cn(
              ...
              rjesenje.length === 0 ||
                isSending ||
                (!isLoggedIn /*|| !isOutputValid*/ && 'cursor-not-allowed border-red bg-white text-red')
            )}
